...
const GaleriGrid = () => {
  ...
  // Update lebar gambar saat resize
  useEffect(() => {
    const updateWidth = () => {
      const width = Math.min(window.innerWidth - 32, 900); // 32px padding
      setImageWidth(width);
    };
    updateWidth();
  ...
  return (
    <div
      className="relative overflow-hidden w-full max-w-[900px] mx-auto h-56 sm:h-72 md:h-[70vh]"
      ...
      >
        {extendedImages.map((img, idx) => (
          <img
            ...
            src={img}
            alt={`Galeri ${idx}`}
            className="object-cover flex-shrink-0 rounded-xl shadow-lg"
